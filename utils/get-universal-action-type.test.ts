...
describe("getUniversalActionType", () => {
  it("should return NONE", async () => {
    ...
    expect(getUniversalActionType({})).toBe("NONE");
    expect(getUniversalActionType({ q: undefined, uriFragment: undefined })).toBe("NONE");
    expect(getUniversalActionType({ uriFragment: "" })).toBe("NONE");
  });
  ...
  it("should return EMBEDDED_URI_FRAGMENT", async () => {
    expect(getUniversalActionType({ q: "", uriFragment: "true" })).toBe("EMBEDDED_URI_FRAGMENT");
    expect(getUniversalActionType({ q: "{}", uriFragment: "true" })).toBe("EMBEDDED_URI_FRAGMENT");
    expect(getUniversalActionType({ q: "somestring", uriFragment: "true" })).toBe("EMBEDDED_URI_FRAGMENT");
  });
});
